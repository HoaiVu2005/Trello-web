...
import { singleFileValidator } from '~/pages/Ultis/validators'
...
import { updateCardDetaisAPI } from '~/apis'
import { updateCardInBoard } from '~/redux/activeBoard/activeBoardSlice'
import { selectCurrentUser } from '~/redux/User/userSlice'
import { CARD_MEMBER_ACTION } from '~/pages/Ultis/constants'
...
const SidebarItem = styled(Box)(({ theme }) => ({
  ...
  '&:hover': {
    ...
    '&.remove': {
      color: theme.palette.mode === 'dark' ? '#000000de' : 'red',
      backgroundColor: theme.palette.mode === 'dark' ? '#90caf9' : '#e9f2ff'
    }
  }
}))
...
function ActiveCard() {
  ...
  const isShowCurrentActiveCard = useSelector(selectIsShowModalActiveCard)
  ...
  const callApiUpdateCard = async (updateData) => {
    const updatedCard = await updateCardDetaisAPI(activeCard._id, updateData)
  ...
  const onAddCardComment = async (commentToAdd) => {
    return await callApiUpdateCard({ commentToAdd })
  }
  ...
  useEffect(() => {
    const onReceiveNewComment = (resComment) => {
      if (!resComment) return
      if (activeCard?._id === resComment?._id) {
        dispatch(updateCurrentActiveCard(resComment))
      }
      dispatch(updateCardInBoard(resComment))
    }
    socketIoInstance.on('BE_COMMENT_TO_DIFFERENT_USER', onReceiveNewComment)
    return () => {
      socketIoInstance.off('BE_COMMENT_TO_DIFFERENT_USER', onReceiveNewComment)
    }
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid xs={12} sm={9}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <CardDescriptionMdEditor cardDecriptionProp={activeCard?.description} handleUpdatCardDescription={onUpdateCardDescription} />
            </Box>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <CardActivitySection cardComment={activeCard?.comments} onAddCardComment={onAddCardComment} />
            </Box>
          </Grid>
          ...
          <Grid xs={12} sm={3}>
            ...
            <Stack direction="column" spacing={1}>
              ...
              {!activeCard?.memberIds?.includes(currentUSer._id) &&
                <SidebarItem className="active" onClick={() => onUpdateCardMembers({ userId: currentUSer?._id, action: CARD_MEMBER_ACTION.ADD })}>
                  <PersonOutlineOutlinedIcon fontSize="small" />
                  Join
                </SidebarItem>
              }
              {activeCard?.memberIds?.includes(currentUSer._id) &&
                <SidebarItem className="remove" onClick={() => onUpdateCardMembers({ userId: currentUSer?._id, action: CARD_MEMBER_ACTION.REMOVE })}>
